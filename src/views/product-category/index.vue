  ...
  <div class="product-category">
    <div style="display: flex; justify-content: space-between">
      <a-form :model="state" layout="inline" autocomplete="off">
        ...
        <a-form-item>
          <a-button type="primary" @click="getProductCategoryList">查询</a-button>
        </a-form-item>
      </a-form>
      <a-button class="create" type="primary" @click="createCategory">创建分类</a-button>
    </div>
...
import * as api from '@/apis/productCategory'
...
const confirm = async (cb: any) => {
  try {
    if (curOperation.value === Operation.Add) {
      await api.createProductCategory(formData.value)
      message.success('添加成功')
    } else if (curOperation.value === Operation.Edit) {
      await api.updateProductCategory(formData.value)
